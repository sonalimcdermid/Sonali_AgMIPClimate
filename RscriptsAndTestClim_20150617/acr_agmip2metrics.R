...
acr_agmip2metrics <- function(infile,jdstart,jdend,column,analysistype,reference,specialoperator){
	...
	write.table(metric, file = fname, sep = ",", col.names = c('Year','Metric'),
            ...
            ## and to read this file back into R one needs to reset working director using setwd('') to whatever directory the metric .csv file above is saved to
#read.table("filename.csv", header = TRUE, sep = ",", row.names = 1)
