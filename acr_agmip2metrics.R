...
# by Yunchul Jung
# at 8/12/2012
# Edited by Sonali McDermid ~early 2014
...
acr_agmip2metrics <- function(infile,jdstart,jdend,column,analysistype,reference,specialoperator){
	...
	#%% read in file
	climdata <- list();
	climdata <- acr_agmipload(paste(rootDir,'data/Climate/Historical/',infile,'.AgMIP',sep=''));
	...
	# The following is a Sonali addition to help automate the metric gathering process
	# scenname = unlist(strsplit(infile, "\\."));
	# scenname = scenname[i]
	# fname <- paste(scenname,'_',analysistype,'_',column,'.csv', sep='');
	# write.table(metric, file = fname, sep = ",", col.names = c('Year','Metric'),
            # qmethod = "double")
            
## To read this file back into R, one needs to reset working director using setwd('') to whatever directory the metric .csv file above is saved to
# metric <- read.table("filename.csv", header = TRUE, sep = ",", row.names = 1)
# for example, metric <- read.table("INCO0XXX.AgMIP_count_8.csv", header = TRUE, sep = ",", row.names = 1)
